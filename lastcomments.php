...
global $user_shortcodes, $euser_pref;
...
if (!check_class($euser_pref['comments_spy']) && (($_GET['page'] == "all_comments") || ($_GET['page'] == "all_pic_comments") || ($_GET['page'] == "all_vid_comments"))) 
{
...
if ($euser_pref['comments_spy_num'] == "" ) {
	$profile_comments_spy_num = 100;
} else {
	$profile_comments_spy_num = $euser_pref['comments_spy_num'];
}
...
if ($euser_pref['comments_spy_pic_size']) {
$pic_td_width = $euser_pref['comments_spy_pic_size'] + 5;
} else {
$pic_td_width = 150;
}
...
if ($euser_pref['commentson'] == "ON" || $euser_pref['commentson'] == "") {
	if ($allcomments == "on") {
		$text .= "<td class='forumheader'><img src='images/comments.png'>&nbsp;&nbsp;&nbsp;<a href='".e_PLUGIN."euser/lastcomments.php?page=all_comments'>".PROFILE_328."</a>";
	} else {
		$text .= "<td class='forumheader'><img src='images/comments.png'>&nbsp;&nbsp;&nbsp;<a href='".e_PLUGIN."euser/lastcomments.php?page=comments'>".PROFILE_328."</a>";
	}
	...
	if ($allcomments == "on") {
		$text .= "<td class='forumheader'><img src='images/pict.png'>&nbsp;&nbsp;&nbsp;<a href='".e_PLUGIN."euser/lastcomments.php?page=all_pic_comments'>".PROFILE_329."</a>";
	} else {
		$text .= "<td class='forumheader'><img src='images/pict.png'>&nbsp;&nbsp;&nbsp;<a href='".e_PLUGIN."euser/lastcomments.php?page=pic_comments'>".PROFILE_329."</a>";
	}
	...
	if ($allcomments == "on") {
		$text .= "<td class='forumheader'><img src='images/vid.png'>&nbsp;&nbsp;&nbsp;<a href='".e_PLUGIN."euser/lastcomments.php?page=all_vid_comments'>".PROFILE_330."</a>";
	} else {
		$text .= "<td class='forumheader'><img src='images/vid.png'>&nbsp;&nbsp;&nbsp;<a href='".e_PLUGIN."euser/lastcomments.php?page=vid_comments'>".PROFILE_330."</a>";
	}
...
if (($euser_pref['commentson'] == "ON" && $comment_type == "prof") || ($euser_pref['pics'] == "ON" && $comment_type == "pics") || ($euser_pref['videos'] == "ON" && $comment_type == "vids")) {
	$sql->mySQLresult = @mysql_query("SELECT com_id FROM ".MPREFIX."euser_com WHERE ".$com_by." com_type='".$comment_type."' ");
	$comnumrows_all = $sql->db_Rows();
	$sql->mySQLresult = @mysql_query("SELECT com_id FROM ".MPREFIX."euser_com WHERE ".$com_by." com_type='".$comment_type."' LIMIT $profile_comments_spy_num ");
	...
	$sql->mySQLresult = @mysql_query("SELECT com_id, com_message, com_date, com_by, com_to, com_extra FROM ".MPREFIX."euser_com WHERE ".$com_by." com_type='".$comment_type."' ORDER BY com_date DESC LIMIT $offset,$rowsPerPage");
	$comm = $sql->db_Rows();
	...
	if ($comm == 0) {
		...
		for ($i = 0; $i < $comm; $i++) {
			...
			if (!ADMIN && USERID != $to_from['user_id']) {
				$friend_db = new db();
				$friend_db->mySQLresult = @mysql_query("SELECT user_friends, user_settings FROM ".MPREFIX."euser WHERE user_id='".$to_from['user_id']."' ");
			...
			if ($comment_type == "vids") {
				...
				$video_query = mysql_query("SELECT * FROM ".MPREFIX."euser_vids WHERE vid_id=".$com['com_extra']." ");
...
//ATW error correction:
			...
			$text .= "<br><table width='100%' class='fborder'>
				<tr>
					...
					if ($comment_type == "prof") {
						$text .= "<a href='".e_PLUGIN."euser/euser.php?id=".$to_from['user_id']."&page=comments'>".$to_from['user_name']."</a></td>";
					} else if ($comment_type == "pics") {
						$text .= "<a href='".e_PLUGIN."euser/euser.php?id=".$to_from['user_id']."&page=images'>".$to_from['user_name']."</a></td>";
						$cucc = PROFILE_332;
					} else if ($comment_type == "vids") {
						$text .= "<a href='".e_PLUGIN."euser/euser.php?id=".$to_from['user_id']."&page=videos'>".$to_from['user_name']."</a>";
						$cucc = PROFILE_333;
					}
					$text .= "<td style='width:".$pic_td_width."px; text-align:right' class='fcaption'>id: #".$comid."</td>
				</tr>
				<td class='forumheader'> ".$newcom."<br>".$online."&nbsp;&nbsp;<a href='euser.php?id=".$com['com_by']."'><b>".$from['user_name']."</b></a></td>
			...
			} else if ((!in_array(USERID, $friendb) && $aof == 1 && USERID != $to_from['user_id']) || ($euser_pref['friends'] == "OFF" && $aof == 1 && USERID != $to_from['user_id']) || ($aof == 1 && !USER)) {
			...
			if ($comment_type == "pics") {
				...
				$pic_width = $euser_pref['comments_spy_pic_size'];
				if ($pic_width == "") $pic_width = 145;
				$kepmeret = getimagesize(e_PLUGIN."euser/userimages/".$to_from['user_id']."/".$dir_neve[0]."/".$kep_even_dir);
				if ($kepmeret[0] > $pic_width) {
					$pic_width = "width = '".$pic_width."'";
				}
				if ($no_user_friend_pics == "1" || $no_user_friend_album == "1") {
					$text .= "<td class='forumheader3' style='text-align: center;'><a href='".e_PLUGIN."euser/euser.php?id=".$to_from['user_id']."&page=images'><img src='".e_PLUGIN."euser/images/kview.png' border='1' ".$pic_width." title='".$to_from['user_name']."".PROFILE_349."' alt='' /></a><br/>".$to_from['user_name']."".PROFILE_349."</td></td>";
				} else if ($no_user_friend_pics == "2") {
					$text .= "<td class='forumheader3' style='text-align: center;'><a href='".e_PLUGIN."euser/euser.php?id=".$to_from['user_id']."&page=images'><img src='".e_PLUGIN."euser/images/kview.png' border='1' ".$pic_width." title='".$to_from['user_name']."".PROFILE_349."' alt='' /></a><br/>".$to_from['user_name']."".PROFILE_349."</td></td>";
				} else {
					$text .= "<td class='forumheader3' style='text-align: center;'><a href='".e_PLUGIN."euser/euser.php?id=".$to_from['user_id']."&page=images&album=".$link_dir_neve."&pic=".$kep_even_dir."'><img src='".e_PLUGIN."euser/userimages/".$to_from['user_id']."/".$dir_neve[0]."/".$kep_even_dir."' border='1' ".$pic_width." title='".$kep_neve."' alt='' /></a><br/>".$kep_neve."</td></tr>";
				}
				}
			if ($comment_type == "vids") {
				$pic_width = $euser_pref['comments_spy_pic_size'];
				...
				if ($no_user_friend_vids == "1") {
					$text .= "<td class='forumheader3' style='text-align: center;'><a href='".e_PLUGIN."euser/euser.php?id=".$to_from['user_id']."&page=videos'><img src='".e_PLUGIN."euser/images/movie_pr.png' border='1' width='".$pic_width."' title='".$to_from['user_name']."".PROFILE_349."' alt='' /></a><br/>".$to_from['user_name']."".PROFILE_349."</td>";
				} else if ($no_user_friend_vids == "2") {
					$text .= "<td class='forumheader3' style='text-align: center;'><a href='".e_PLUGIN."euser/euser.php?id=".$to_from['user_id']."&page=videos'><img src='".e_PLUGIN."euser/images/movie_pr.png' border='1' width='".$pic_width."' title='".$to_from['user_name']."".PROFILE_349."' alt='' /></a><br/>".$to_from['user_name']."".PROFILE_349."</td>";
				} else {
					$text .= "<td class='forumheader3' style='text-align: center;'><a href='".e_PLUGIN."euser/euser.php?id=".$to_from['user_id']."&page=videos&vid=".$com['com_extra']."'><img src=".$video_kep." width='".$pic_width."' height='".$pic_height."' alt='' title='".$video_neve."' /></a><br/>".$video_neve."</td></tr>";
				}
			}
			...
			if ($allcomments == "on") {
				$text .= "<div align='right' class='smallblacktext'><a href='euser.php?id=".$com['com_by']."&page=comments'>".PROFILE_137a."".$from['user_name']."".PROFILE_137b."</a>";
