...
$numquery = mysql_query("SELECT user_id FROM ".MPREFIX."euser WHERE user_lastupdated != ''  LIMIT 100 ");
...
$query = mysql_query("SELECT u.user_id, u.user_name, u.user_image, a.user_custompage, a.user_lastupdated FROM ".MPREFIX."user u, ".MPREFIX."euser a WHERE a.user_lastupdated != '' AND a.user_id = u.user_id ORDER BY a.user_lastupdated DESC LIMIT $offset,$rowsPerPage");
...
for ($i = 0; $i <= $rows; $i++) {
	...
	if ($row['user_id'] != "") {
		...
		$text .= "<td class='forumheader'><center>
			<a style=\"text-decoration: none;\" href='".e_PLUGIN."euser/euser.php?id=".$row['user_id']."'><img src='".$avatar."' ".$avwidth." ".$avheight." /><br/>".$row["user_name"]."</a><br/>".$profile_mod."<br/>".date("Y.m.d", $row['user_lastupdated'])."</center></td>";
		if ($euser_pref['updatedtotal_col'] == '') {
			$profile_updatedtotal_col = '4';
		} elseif ($euser_pref['updatedtotal_col'] > '8') {
			$profile_updatedtotal_col = '8';
		} else {
			$profile_updatedtotal_col = $profile_updatedtotal_col;
		}
		if ($count == $euser_pref['updatedtotal_col']) {
