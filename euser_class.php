...
*/
//    public function __construct(e_menu $parent) {
//         $this->emenuInstance = $parent;
    }
...
if($user_admin==1){$usercol='style="text-decoration: none; font-weight:bold; color:'.$euser_pref['admincolour'].';"';}
}

if($euser_pref['headadminactive']==1){
if($user_admin==1 && $user_perms=='0'){$usercol='style="text-decoration: none; font-weight:bold; color:'.$euser_pref['headadmincolour'].';"';}
}

if($euser_pref['modactive']==1){
if($usermodr==1 && $user_admin<>1){$usercol='style="text-decoration: none; font-weight:bold; color:'.$euser_pref['modcolour'].';"';}
}

if($euser_pref['memactive']==1){
if($usermodr==0 && $user_admin<>1){$usercol='style="text-decoration: none; color:'.$euser_pref['memcolour'].';"';}	
}
	
if($usercol=="" || $usercol=='style="text-decoration: none; color:'.$euser_pref['memcolour'].';"'){ // user classes	
...
global $tp;
	$colourkey='<div style="text-align:left;" class="mediumtext"><span class="fa-stack">'.$tp->toGlyph('user').$tp->toGlyph('info-sign').'</span><b>'.ONLINENOW_7.'</b>:</div>';
//	$colourkey='<div style="text-align:left;" class="mediumtext">'.e107::getParser()->toGlyph('user').e107::getParser()->toGlyph('info-sign').'<b>'.ONLINENOW_7.'</b>:</div>';
	...
	if($euser_pref['headadminactive']==1){
//		$colourkey.='<div style="text-align:left; font-weight:bold; color:'.$euser_pref['headadmincolour'].';">'.ONLINENOW_1.'</div>';
		$colourkey.='<span style="text-align:left; font-weight:bold; color:'.$euser_pref['headadmincolour'].';"><nobr>'.ONLINENOW_1.'</nobr></span>';
	}  
	 
	if($euser_pref['adminactive']==1){  
//		$colourkey.='<div style="text-align:left; font-weight:bold; color:'.$euser_pref['admincolour'].';">'.ONLINENOW_8.'</div>';
		$colourkey.=', <span style="text-align:left; font-weight:bold; color:'.$euser_pref['admincolour'].';"><nobr>'.ONLINENOW_8.'</nobr></span>';
	}
	
	if($euser_pref['modactive']==1){
//		$colourkey.='<div style="text-align:left; font-weight:bold; color:'.$euser_pref['modcolour'].';">'.ONLINENOW_10.'</div>';
		$colourkey.=', <span style="text-align:left; font-weight:bold; color:'.$euser_pref['modcolour'].';"><nobr>'.ONLINENOW_10.'</nobr></span>';
	}
	
	$colourkey.=$classdata;
	
	if($euser_pref['memactive']==1){
//		$colourkey.='<div style="text-align:left; color:'.$euser_pref['memcolour'].';">'.ONLINENOW_9.'</div>';	
		$colourkey.=', <span style="text-align:left; color:'.$euser_pref['memcolour'].';"><nobr>'.ONLINENOW_9.'</nobr></span>';	
	}
...
if($layout==1){
	...
	if($euser_pref['headadminactive']==1){
//		$colourkey.='<div style="text-align:left; font-weight:bold; color:'.$euser_pref['headadmincolour'].';">'.ONLINENOW_1.'</div>';
		$colourkey.='<span style="text-align:left; font-weight:bold; color:'.$euser_pref['headadmincolour'].';">'.ONLINENOW_1.'</span>, ';
	}  
	 
	if($euser_pref['adminactive']==1){  
//		$colourkey.='<div style="text-align:left; font-weight:bold; color:'.$euser_pref['admincolour'].';">'.ONLINENOW_8.'</div>';
		$colourkey.='<span style="text-align:left; font-weight:bold; color:'.$euser_pref['admincolour'].';">'.ONLINENOW_8.'</span>, ';
	}
	
	if($euser_pref['modactive']==1){
//		$colourkey.='<div style="text-align:left; font-weight:bold; color:'.$euser_pref['modcolour'].';">'.ONLINENOW_10.'</div>';
		$colourkey.='<span style="text-align:left; font-weight:bold; color:'.$euser_pref['modcolour'].';">'.ONLINENOW_10.'</span>, ';
	}
	...
	if($euser_pref['memactive']==1){
//		$colourkey.='<div style="text-align:left; color:'.$euser_pref['memcolour'].';">'.ONLINENOW_9.'</div>';	
		$colourkey.='<span style="text-align:left; color:'.$euser_pref['memcolour'].';">'.ONLINENOW_9.'</span>';	
	}
	...
	if($euser_pref['headadminactive']==1){
//		$colourkey.='<div style="text-align:left;"><span style="text-align:left; font-weight:bold; color:'.$euser_pref['headadmincolour'].';">'.ONLINENOW_1.'</span>, ';  
		$colourkey.='<span style="text-align:left;"><span style="text-align:left; font-weight:bold; color:'.$euser_pref['headadmincolour'].';">'.ONLINENOW_1.'</span>, ';  
	} 
	...
	if($euser_pref['memactive']==1){
		$colourkey.='<span style="text-align:left; color:'.$euser_pref['memcolour'].';">'.ONLINENOW_9.'</span></div>';
	}
...
function Create_sound_dropdown($Selname,$SelectedVal){ // Sound setup in admin
	...
	$ret_text.='<option value="none"'.$sel.'>' .EUSER_LOGIN_MENU_A61. '</option>';
	
	$sel='';
	
	$soundhandle=opendir(e_PLUGIN.'euser/sounds/');
...
function Create_eorder_dropdown($Selname,$SelectedVal){ // extra order of cache in admin
	...
	$sql=new db;
	$checkcacheno = $sql -> db_Count("euser_cache", "(*)", "WHERE type ='extraorder'");
...
$pdata = eregi_replace('flashchat',EUSER_LOGIN_MENU_L62, $pagedata);
$pdata = eregi_replace('getxml',EUSER_LOGIN_MENU_L62, $pdata);									
$pdata = eregi_replace('sig',EUSER_LOGIN_MENU_L71, $pdata);
$pdata = eregi_replace('bf2142BF2 Sig Gen',EUSER_LOGIN_MENU_L85, $pdata);
$pdata = eregi_replace('create_sig Gen',EUSER_LOGIN_MENU_L85, $pdata);
$pdata = eregi_replace('BF2 Sig Gennup',EUSER_LOGIN_MENU_L3, $pdata);
$pdata = eregi_replace('bf2stats',EUSER_LOGIN_MENU_L72, $pdata);
$pdata = eregi_replace('rss',EUSER_LOGIN_MENU_L73, $pdata);
$pdata = eregi_replace('ratepic',EUSER_LOGIN_MENU_L74, $pdata);
//$pdata = eregi_replace('thumbnails',EUSER_LOGIN_MENU_L74, $pdata);
$pdata = eregi_replace('displayimage',EUSER_LOGIN_MENU_L74, $pdata);
$pdata = eregi_replace('ecard',EUSER_LOGIN_MENU_L74, $pdata);
$pdata = eregi_replace('addfav',EUSER_LOGIN_MENU_L74, $pdata);					                
$pdata = eregi_replace('comment.comment.news.', ONLINE_EL12. ' - ', $pdata);
$pdata = eregi_replace('comment.comment.poll.', EUSER_LOGIN_MENU_L63, $pdata);
$pdata = eregi_replace('comment.comment', 'Comment ', $pdata);	
$pdata = eregi_replace('chatbox2_control', EUSER_LOGIN_MENU_L123, $pdata);	
$pdata = eregi_replace(SITEURL.'Coppermine', EUSER_LOGIN_MENU_L74, $pdata);	
...
//var_dump ($this->template['label0'] && $this->template['label']);
//var_dump ($this->template);
//var_dump ($this->template['label0']);
//var_dump ($this->template['label']);

      if ($this->template['label0'] && $this->template['label']){
        $template = ($total==0?$this->template['label0']:$this->template['label']);      
//      $tmp = array();
//      var_dump ($template);
      $tmp['VALUE']=$total;
    	return e107::getParser()->parseTemplate($template, true, $tmp);
      }
      return $total;
...
//class eusermenu extends e_menu
//{
			function getmenuparm($eMenuArea, $menu = ''){

//var_dump ($eMenuArea->getDataLegacy());    
//      $newmenu = new e_menu;
//  $this->obj = new e_menu();
//  var_dump ($newmenu);
//  var_dump (e107::getmenu()->renderMenu("euser", $menu));
//  var_dump (e107::getmenu()->pref());
//  var_dump ($eMenuArea);
//    $loginPrefs = e107::getConfig('menu')->getPref();
//  var_dump ($loginPrefs);
//  var_dump (e107::getConfig('menu')->getPref('euser_all'));
//var_dump(parent::getDataLegacy());

//$eMenuArea = self::getDataLegacy();
...
//          var_dump (e107::unserialize($row['menu_parms']));
          return (e107::unserialize($row['menu_parms']));
