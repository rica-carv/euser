...
function euser_LocalWrappers($text, array $wrappers)
{
    foreach ($wrappers as $sc => $wrap)
    {
        // Escapar corretamente o shortcode para regex
        $scRegex = preg_quote($sc, '#');

        // Match exato do shortcode (com chaves)
//        $pattern = '#\{'.$scRegex.'\}#';
        // {SC} ou {SC:...}
        $pattern = '#\{'.$scRegex.'(?:\:[^}]*)?\}#';

        $text = preg_replace_callback($pattern, function($m) use ($wrap) {

            $shortcode = $m[0]; // ex: {EUSER_POSTS}

            // 1️⃣ Avaliar o shortcode isoladamente
            $out = $this->tp->parseTemplate($shortcode, true, $this);

            // 2️⃣ Se não houver output → nada (como o core)
            if ($out === null || trim($out) === '') {
                return '';
            }

            // 3️⃣ Aplicar wrapper
            return str_replace('{---}', $out, $wrap);
...
/*
    protected function render_user_sendpm($parms = null)
    {
//		var_dump ($parm);
