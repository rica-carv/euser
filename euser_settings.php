...
$doc = DOMDocument::loadHTML($tp->parseTemplate("{USER_AVATAR=$id}", TRUE, $user_shortcodes));
...
$text .="<tr><td {$main_colspan} width='20%' class='forumheader3'>";
	$text .= "<img src ='images/key.png' style='border: 0px solid black; width: 16px; height: 16px; float: left; margin-right: 3px' alt = ''><a href='euser_settings.php?page=settings".$luid."'>".PROFILE_141."</a><br/>";
...
$title = $tp->parseTemplate($caption, TRUE, $user_shortcodes);

function new_user_row($member_id) {
	global $sql;
	$sql -> db_Select("euser", "*", "user_id='".$member_id."'");
	$count = $sql -> db_Rows();
	if ($count == 0) {
